...
AVA_DATASET_PATH = "/path/to/dataset/"  # CHANGEME
...
class TubeDataset(object):
    ...
    def __init__(self, dname, split=1):
        assert dname in ['UCFSports','JHMDB','UCF101', 'AVA'], "Unknown dataset name"
        self.NAME = dname
        self.SPLIT = split
...
class AVA(TubeDataset):
    def __init__(self, split=1):
        assert split == 1, "We only have one split for now"
        super(AVA, self).__init__('AVA',1)

    def vidfile(self,v,i):
        return os.path.join(AVA_DATASET_PATH, "frames/clips/", v)

    def imfile(self,v,i):
        #print(os.path.join(AVA_DATASET_PATH, "frames", "Frames", v, "{:0>5}.jpg".format(i)))
        return os.path.join(AVA_DATASET_PATH, "frames", "Frames", v, "{:0>5}.jpg".format(i))
    
    def flowfile(self,v,i):
        v = v.split("/")
        #print(os.path.join(AVA_DATASET_PATH, "frames", "Farneback", v[0], "{:0>4}".format(v[1]), "{:0>5}.jpg".format(i)))
        return os.path.join(AVA_DATASET_PATH, "frames", "Farneback", v[0], "{:0>4}".format(v[1]), "{:0>5}.jpg".format(i))

    def frame_format(self,v,i):
        return os.path.join(v)

def GetDataset(dname):
    assert dname in ['UCFSports', 'JHMDB', 'JHMDB2', 'JHMDB3', 'UCF101', 'AVA'], "Unknown dataset " + dname
